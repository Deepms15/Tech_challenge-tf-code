...
resource "aws_instance" "this" {
    ...
    user_data                     = length(var.user_data) > 1? element(var.user_data, count.index) : element(var.user_data, 0)
    network_interface { 
  ...
  volume_tags = merge(
        {
            Name= var.instance_count>1 ? "vol-${var.instance_name}-${count.index}":"vol-${var.instance_name}"
        }, 
            var.common_tags,
    )
